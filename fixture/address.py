...
class AddressHelper:
    ...
    def delete_address(self):                                               # Удаление контакта
        ...
        wd.find_element_by_css_selector("img[alt=\"Edit\"]").click()        # Выбор удаляемого контакта
        wd.find_element_by_xpath("//div[@id='content']/form[2]/input[2]").click()   # Подтверждение удаления

    def edit_address(self, address):                                        # Изменение параметров контакта
        wd = self.app.wd                                                    # Получить доступ к web-драйверу
        wd.find_element_by_css_selector("img[alt=\"Edit\"]").click()        # Выбор изменяемого контакта
        # Заполнение параметров контакта
        wd.find_element_by_name("firstname").click()
        wd.find_element_by_name("firstname").clear()
        wd.find_element_by_name("firstname").send_keys(address.first_name)
        ...
        wd.find_element_by_name("homepage").clear()
        wd.find_element_by_name("homepage").send_keys(address.web_homepage)
        if not wd.find_element_by_xpath("//div[@id='content']/form[1]/select[1]//option[" + str(address.birthday_day + 2) + "]").is_selected():
            wd.find_element_by_xpath("//div[@id='content']/form[1]/select[1]//option[" + str(address.birthday_day + 2) + "]").click()
        if not wd.find_element_by_xpath("//div[@id='content']/form[1]/select[2]//option[" + str(address.birthday_month + 2) + "]").is_selected():
            wd.find_element_by_xpath("//div[@id='content']/form[1]/select[2]//option[" + str(address.birthday_month + 2) + "]").click()
        wd.find_element_by_name("byear").click()
        wd.find_element_by_name("byear").clear()
        wd.find_element_by_name("byear").send_keys(address.birthday_year)
        if not wd.find_element_by_xpath("//div[@id='content']/form[1]/select[3]//option[" + str(address.anniversary_day + 2) + "]").is_selected():
            wd.find_element_by_xpath("//div[@id='content']/form[1]/select[3]//option[" + str(address.anniversary_day + 2) + "]").click()
        if not wd.find_element_by_xpath("//div[@id='content']/form[1]/select[4]//option[" + str(address.anniversary_month + 2) + "]").is_selected():
            wd.find_element_by_xpath("//div[@id='content']/form[1]/select[4]//option[" + str(address.anniversary_month + 2) + "]").click()
        wd.find_element_by_name("ayear").click()
        wd.find_element_by_name("ayear").clear()
        wd.find_element_by_name("ayear").send_keys(address.anniversary_year)
        wd.find_element_by_name("address2").click()
