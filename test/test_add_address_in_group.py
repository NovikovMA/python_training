...
def test_add_address_in_group(app, orm):
    ...
    if len(orm.get_addresses_not_in_group(group)) == 0:                     # Проверка наличия хотябы одного контакта, не включенного в группу
    ...
    addresses_not_in_group = orm.get_addresses_not_in_group(group)          # Получение списка контактов, не включенных в группу
    count = 1                                                               # Количество контактов для добавления в группу (random.randrange(len(addresses_not_in_group)))
    address_list = [random.choice(addresses_not_in_group) for i in range(count)]    # Выбор контактов для добавления в группу
                                                                            # Иключение повторений контактов
    count = len(address_list)                                               # Количество контактов после исключения повторений
    app.address.add_to_group_by_index(address_list, group_index)            # Добавление контактов в группу
    ...
    old_addresses_in_group = old_addresses_in_group + address_list          # Добавление контакта в старый список группы
